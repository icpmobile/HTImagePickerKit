...
#import "HTImageDragView.h"
#import "HTImageButton.h"

@interface HTImageDragView () <UIGestureRecognizerDelegate>
@property (nonatomic,strong) HTImageButton *addButton;
//所有的照片按钮
...
@implementation HTImageDragView
#pragma mark - init
    ...
    if (self) {
        ...
        self.addButton.buttonClickedBlock = ^(){
            ...
            if ([weakSelf.dragViewDelegete respondsToSelector:@selector(imageDragViewAddButtonClicked:)]) {
                [weakSelf.dragViewDelegete imageDragViewAddButtonClicked:weakSelf.send];
            }
        };
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer{
    ...
    if ([view isKindOfClass:[HTImageButton class]]) {
        HTImageButton *button = (HTImageButton *)gestureRecognizer.view;
...
{
    //获取移动的按钮
    HTImageButton *button = (HTImageButton *)longPress.view;
    ...
    switch (longPress.state) {
        ...
        case UIGestureRecognizerStateEnded:{
            ...
            if ([self.dragViewDelegete respondsToSelector:@selector(imageDragViewDidMoveButtonFromIndex:toIndex:send:)] && newIndex != self.originIndex) {
                [self.dragViewDelegete imageDragViewDidMoveButtonFromIndex:self.originIndex toIndex:newIndex send:self.send];
            }
        }
            break;
        default:
        ...
        for (int i = 0; i<self.allButtons.count; i++) {
            [UIView animateWithDuration:0.25 animations:^{
                for (int i = 0; i<self.allButtons.count; i++) {
                    ...
                    HTImageButton *button = self.allButtons[i];
                    //过滤不需要进行布局的按钮
                    BOOL isSame = NO;
                    for (HTImageButton *imageButton in buttons) {
...
- (void)addImage:(UIImage *)image{
    ...
    //添加一个照片按钮
    HTImageButton *button = [[HTImageButton alloc] initWithFrame:CGRectZero];
    ...
    button.deleteButtonClickedBlock = ^(){
        ...
        //回调
        if ([weakSelf.dragViewDelegete respondsToSelector:@selector(imageDragViewDeleteButtonClickedAtIndex:send:)]) {
            [weakSelf.dragViewDelegete imageDragViewDeleteButtonClickedAtIndex:index send:self.send];
        }
    };
    //点击事件
    button.buttonClickedBlock = ^(){
        //回调
        NSInteger index = [weakSelf.allButtons indexOfObject:weakbutton];
        if ([weakSelf.dragViewDelegete respondsToSelector:@selector(imageDragViewButtonClickedAtIndex:send:)]) {
            [weakSelf.dragViewDelegete imageDragViewButtonClickedAtIndex:index send:self.send];
        }
    };
...
- (void)removeAllImages{
    for (NSInteger i = self.allButtons.count - 1; i >= 0; --i) {
        HTImageButton *button = self.allButtons[i];
        if (!button.isAddButton) {
            [self.allButtons removeObjectAtIndex:i];
